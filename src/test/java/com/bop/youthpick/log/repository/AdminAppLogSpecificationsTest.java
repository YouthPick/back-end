...
class AdminAppLogSpecificationsTest {
    ...
    private void saveAppLog(String level, String message, String traceId, LocalDateTime createdAt) {
        ApplicationLog appLog = BeanUtils.instantiateClass(ApplicationLog.class);
        ...
        ApplicationLog saved = appLogRepository.saveAndFlush(appLog);

        entityManager
                .getEntityManager()
                .createNativeQuery("UPDATE application_logs SET created_at = ?1 WHERE id = ?2")
    ...
    void logLevel로_필터링한다() {
        List<ApplicationLog> result =
                appLogRepository.findAll(
                        AdminAppLogSpecifications.filter("WARN", null, null, null));

        assertThat(result).extracting(ApplicationLog::getTraceId).containsExactly("trace-2");
    }
    ...
    void keyword로_message_traceId_uri_부분일치를_검색한다() {
        ...
        assertThat(result).extracting(ApplicationLog::getTraceId).containsExactly("trace-1");
    }
