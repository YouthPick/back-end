...
/**
 * log4j2.xml의 JDBC Appender가 참조하는 app_logs 전용 소형 커넥션 풀. Spring 메인 데이터소스와 분리해 Spring 초기화 전에도 동작한다.
 */
public final class AppLogDataSource {

    private static volatile HikariDataSource dataSource;

    private AppLogDataSource() {}

    public static DataSource getDataSource() {
        HikariDataSource local = dataSource;
        if (local == null) {
            synchronized (AppLogDataSource.class) {
                local = dataSource;
                if (local == null) {
                    local = build();
                    dataSource = local;
    ...
    private static HikariDataSource build() {
        ...
        config.setPoolName("app-log-jdbc-appender");
