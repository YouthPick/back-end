...
/**
 * categories/keywords는 온보딩 화면에서 체크박스로 다중 선택되는 값이라 콤마 문자열이 아니라 배열로 받는다. 콤마 결합(저장 포맷)은 서비스 계층 책임이다.
...
public record OnboardingProfileRequest(
        ...
        @NotBlank(message = "거주 지역은 필수입니다.") @Size(max = 10, message = "지역 코드 형식이 올바르지 않습니다.")
                String regionCode,
        @Size(max = 16, message = "취업상태 코드 형식이 올바르지 않습니다.") String employmentStatus,
        @Size(max = 16, message = "학력 코드 형식이 올바르지 않습니다.") String educationLevel,
