 ...
 * 사용하면 된다. 상태 코드가 필요한 경우, ResponseEntity<ApiResponse<?>> ?에 타입을 넣고 맞춰서 사용. 페이지 목록 응답은 {@code
 * ok(data, page)}로 감싸면 meta에 page/totalCount/totalPages가 담긴다. meta가 없으면 응답 JSON에서 생략된다. 에러 응답은 이
 * 타입이 아니라 {@code global.error.ErrorResponse} 형식으로 내려간다.
 */
