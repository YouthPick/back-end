...
public class PolicyApplicationChecklist extends BaseEntity {
    ...
    // 컬럼/필드명은 content이지만, 이걸 감싸는 DTO 쪽(PolicyApplicationCreateChecklistRequest 등)은
    // 전부 message라는 이름을 쓴다 — 같은 값인데 계층마다 이름이 달라서 코드를 오갈 때 헷갈리기 쉽다.
    @Column(columnDefinition = "TEXT", nullable = false)
    private String content;

    // Lombok @Getter가 boolean 필드 checked에 대해 isChecked()를 생성한다 —
    ...
    // PolicyApplicationChecklistService.delete()에서 호출하는 개별 소프트 삭제.
    // 여러 항목을 한 번에 지우는 경로(부모 신청관리 재등록 시)는 이 메서드를 쓰지 않고
    // PolicyApplicationChecklistRepository.softDeleteAllByApplicationId()가 벌크 UPDATE로 처리한다.
    public void delete() {
