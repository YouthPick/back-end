 ...
 *     내려간다(api-design.md). 이 화이트리스트 문자열은 ApplicationStatus enum 상수 이름과 정확히 일치해야 하고,
 *     PolicyApplicationController.changeStatus()의 {@code @Pattern}과도 중복 관리된다. 이 화이트리스트와 enum이 어긋나도
 *     컨트롤러의 parseStatus()가 IllegalArgumentException을
