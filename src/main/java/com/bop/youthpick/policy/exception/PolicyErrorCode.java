...
public enum PolicyErrorCode implements ErrorCode {
    /** PolicyApplicationService.create()에서 policyRepository.findById(policyId)가 비었을 때 던진다. */
    POLICY_NOT_FOUND("P001", "일치하는 정책이 존재하지 않습니다.", HttpStatus.NOT_FOUND),

    /**
     * PolicyApplicationService.create()에서 두 번 쓰인다: ①활성 상태인 기존 신청이 있을 때(중복 등록 시도), ②동시 요청으로
     * UNIQUE(user, policy) 제약을 위반한 DataIntegrityViolationException을 잡았을 때 — 같은 도메인 에러로 통일.
     */
     ...
     * 경로), PolicyApplicationChecklistService에서도 같은 목적으로 재사용된다.
     */
    POLICY_APPLICATION_NOT_FOUND("P004", "정책 관리 항목을 찾을 수 없습니다.", HttpStatus.NOT_FOUND),

    /**
     * PolicyApplicationChecklistService 전용 — 이 파일이 속한 policy-application 핵심 흐름(create/changeStatus
     * 등)에선 쓰이지 않는다.
     */
    CHECKLIST_NOT_FOUND("P005", "체크리스트 항목을 찾을 수 없습니다.", HttpStatus.NOT_FOUND),

    /**
     * PolicyApplicationService.validateWithinPolicyDeadline()에서 던진다 — create()의 resolveEndAt()과
    ...
    /**
     * PolicyApplicationController.parseStatus()에서 던진다 — DTO/{@code @RequestParam}의
     * {@code @Pattern}이 이미 걸러주는 값이라 평소엔 발생하지 않지만, ApplicationStatus enum과 두 곳의 {@code @Pattern}
