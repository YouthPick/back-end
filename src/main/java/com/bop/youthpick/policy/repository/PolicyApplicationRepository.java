...
public interface PolicyApplicationRepository
    ...
    /**
     * PolicyApplicationService.getApplications()(목록 조회)에서 사용. {@code @EntityGraph}로 policy 연관관계를 함께
     * fetch해서, PolicyApplicationResponse.from()이 policy.getTitle() 등을 부를 때 N+1 쿼리가 나지 않게 한다.
     */
     ...
     * 있는지부터 확인한다(테이블의 UNIQUE(user_id, policy_id) 제약과 짝을 이루는 조회).
     */
