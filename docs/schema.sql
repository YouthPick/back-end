...
CREATE TABLE user_profiles (
    ...
    employment_status  VARCHAR(16)  NOT NULL COMMENT '온보딩 취업상태 코드(UNEMPLOYED 등). jobCd 변환은 YouthPolicyCodeMapper (REC 취업 15점)',
    education_level    VARCHAR(16)  NOT NULL COMMENT '온보딩 학력 코드(UNIV_GRADUATE 등). schoolCd 변환은 YouthPolicyCodeMapper (REC 학력 10점)',
    merry_status       VARCHAR(16)  NULL     COMMENT '결혼여부 코드 (선택)',
    major              VARCHAR(255) NULL     COMMENT '전공 콤마목록, plcyMajorCd와 형식 통일 (선택)',
    special_condition  VARCHAR(500) NULL     COMMENT '특화조건 콤마목록 (선택)',
    income             INT          NULL     COMMENT '연소득(만원), policies.income_max_amount와 단위 통일 (선택)',
